def init_parameters():
    ...
    # Training and test
    params['method'] = 'sarsa' # 'qlearning' | 'sarsa' | 'ppo' | 'manhattan'
    params['train'] = True #  'True' | 'False' --- false = test run, which does not change NN   
    params['show_gui'] = False # disabling the GUI improves training speed


    # Hyperparameter for all RL methods
    params['train_episodes'] = 400 # episodes for training. should be atleast 1000
    ...
    params['ppo_update_timestep'] = 100 # update the policy every x game steps
    ...
    # game settings
    params['game_field_size'] = 12 # must be even and atleast 6
